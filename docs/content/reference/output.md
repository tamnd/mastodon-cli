...
describes what users get. Pick a format with `-o`, or let mastodon choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mastodon <command> -o table   # aligned columns for reading
mastodon <command> -o jsonl   # one JSON object per line, for piping
mastodon <command> -o json    # a single JSON array
mastodon <command> -o csv     # spreadsheet friendly
mastodon <command> -o tsv     # tab-separated
mastodon <command> -o url     # just the URL column
mastodon <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mastodon <command> --fields id,title,url
```
...
```bash
mastodon <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mastodon <command>            # a table, because this is a terminal
mastodon <command> | wc -l    # JSONL, because this is a pipe
```
